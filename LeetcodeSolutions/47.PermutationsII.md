...
public:
    ...
    void permute_helper(vector<int> nums, vector<int> current_permutation){
        if(nums.size() == 0){
            //Push only if not already discovered
            if(find(permutations.begin(),permutations.end(),current_permutation) == permutations.end())permutations.push_back(current_permutation);
            return;
        }
        else{
            for(int i = 0; i<nums.size(); i++){
